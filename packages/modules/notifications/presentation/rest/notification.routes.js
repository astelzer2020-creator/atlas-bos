...
import { inboxQuerySchema, notificationParamsSchema, organizationParamsSchema, sendNotificationBodySchema, updatePreferencesBodySchema, } from './schemas.js';
...
function sendResult(reply, result, successStatus = 200, wrap = false) {
    if (!result.ok) {
...
export async function registerNotificationRoutes(fastify, deps) {
    fastify.get('/v1/organizations/:organizationId/notifications', async (request, reply) => {
    ...
    fastify.post('/v1/organizations/:organizationId/notifications', async (request, reply) => {
        const userId = await requireAuth(request, reply, deps.authenticate);
        if (userId === null) {
            return;
        }
        ...
        const result = await deps.notificationService.sendNotification(orgIdResult.value, {
    ...
    fastify.patch('/v1/organizations/:organizationId/notifications/:id/read', async (request, reply) => {
        const userId = await requireAuth(request, reply, deps.authenticate);
        if (userId === null) {
            return;
        }
        ...
        const result = await deps.notificationService.markAsRead(orgIdResult.value, params.data.id, userId);
        return sendResult(reply, result, 200, true);
    });
    fastify.get('/v1/organizations/:organizationId/notification-preferences', async (request, reply) => {
        const userId = await requireAuth(request, reply, deps.authenticate);
        if (userId === null) {
            return;
        }
        ...
        const preferences = await deps.notificationService.getPreferences(orgIdResult.value, userId);
        return reply.status(200).send({
    ...
    fastify.patch('/v1/organizations/:organizationId/notification-preferences', async (request, reply) => {
        const userId = await requireAuth(request, reply, deps.authenticate);
        if (userId === null) {
            return;
        }
        ...
        const preferences = await deps.notificationService.updatePreferences(orgIdResult.value, userId, {
