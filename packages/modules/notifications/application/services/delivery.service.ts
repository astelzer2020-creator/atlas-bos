...
import { ok, type OrganizationId, type Result } from '@atlas/shared-kernel';
...
export class DeliveryService {
  ...
  async queueDelivery(
    ...
    enabledChannelTypes: readonly ('in_app' | 'email')[],
  ): Promise<Result<QueueDeliveryResult, never>> {
    ...
    for (const channelType of enabledChannelTypes) {
      ...
      const existing = await this.deps.notificationRepository.findDeliveryByIdempotencyKey(
        deliveryKey,
      );
  ...
  async processPendingDeliveries(
    limit = 50,
  ): Promise<Result<ProcessDeliveriesResult, never>> {
    const log = this.deps.logger.child({
      module: 'notifications',
    ...
    for (const delivery of pending) {
      ...
      const notification = await this.deps.notificationRepository.findById(
        delivery.tenantId,
      ...
      let result: Result<NotificationDeliveryRecord, never>;
  ...
  private async syncNotificationStatus(
    tenantId: OrganizationId,
    notificationId: string,
  ): Promise<void> {
    const deliveries =
      await this.deps.notificationRepository.listDeliveriesByNotificationId(notificationId);
