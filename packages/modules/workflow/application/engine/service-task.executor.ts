...
async function executeWebhook(config: Record<string, unknown>): Promise<ServiceTaskResult> {
  const url = config.url;
  const method = (typeof config.method === 'string' ? config.method.toUpperCase() : 'POST') as
    | 'GET'
    | 'POST'
    | 'PUT'
    | 'PATCH'
...
function executeNotify(config: Record<string, unknown>): ServiceTaskResult {
 ...
 */
export async function executeServiceTask(context: ServiceTaskContext): Promise<ServiceTaskResult> {
  const service =
    typeof context.config.service === 'string' ? context.config.service : 'noop';
